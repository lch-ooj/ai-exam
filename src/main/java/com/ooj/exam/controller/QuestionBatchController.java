...
public class QuestionBatchController {
    ...
    public ResponseEntity<byte[]> downloadTemplate() {
        try {
            byte[] templateData = ExcelUtil.generateTemplate();

            ResponseEntity<byte[]> responseEntity = ResponseEntity.ok()//返回状态码200
                    //设置响应头。固定写法——附件。告诉浏览器不要打开，是下载。（浏览器默认会打开支持的文件格式）
    ...
    public Result<List<QuestionImportVo>> previewExcel(
        ...
        List<QuestionImportVo> questionImportVoList = questionService.parseExcel(file);
    ...
    public Result<String> importQuestions(@RequestBody List<QuestionImportVo> questions) {
        int successCount = questionService.importQuestions(questions);
        return Result.success("成功导入%s道题目".formatted(successCount));
    }
